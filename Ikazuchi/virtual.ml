...
let rec g env = function (* 式の仮想マシンコード生成 (caml2html: virtual_g) *)
  ...
  | Closure.Float(d) ->
	...
	with Not_found ->
	  ...
	  l in
      let x = Id.genid "l" in
      Let((x, Type.Int), SetL(l), Ans(FMovD(x)))
  ...
  | Closure.Get(x, y) -> (* 配列の読み出し (caml2html: virtual_get) diff*)
      ...
      | Type.Array(Type.Float) -> Ans(LdDF(x, V(y)))
      | Type.Array(_) -> Ans(Ld(x, V(y)))
      | _ -> assert false)
  | Closure.Put(x, y, z) ->
      (match M.find x env with
      | Type.Array(Type.Unit) -> Ans(Nop)
      | Type.Array(Type.Float) -> Ans(StDF(z, x, V(y)))
      | Type.Array(_) -> Ans(St(z, x, V(y)))
      | _ -> assert false)
  | Closure.ExtArray(Id.L(x)) -> Ans(SetA(Id.L("min_caml_" ^ x)))
